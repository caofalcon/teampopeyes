...
static counter_t q1_1cycle = 0; 
static counter_t q1_2cycle = 0;
static counter_t q2_1cycle = 0; 
static counter_t q2_2cycle =0;  
static counter_t sim_num_RAW_hazard_q2 = 0;
static counter_t reg_ready[MD_TOTAL_REGS];
static counter_t q2_reg_ready[MD_TOTAL_REGS];
int r_out[2], r_in[3];
...
sim_reg_stats(struct stat_sdb_t *sdb)
{
  ...
  stat_reg_counter(sdb, "q1_1cycle", "q1_1cycle(q1)", &q1_1cycle, q1_1cycle, NULL);  

  stat_reg_counter(sdb, "q1_2cycle", "q1_2cycle(q1)", &q1_2cycle, q1_2cycle, NULL);  

  stat_reg_counter(sdb, "q2_1cycle", "q2_1cycle(q2)", &q2_1cycle, q2_1cycle, NULL);  

  stat_reg_counter(sdb, "q2_2cycle", "q2_2cycle(q2)", &q2_2cycle, q2_2cycle, NULL);  
  ...
  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q1",
		   ...
		   "(q1_1cycle + 2*q1_2cycle + sim_num_insn)/sim_num_insn", NULL);

  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q2",
		   "CPI from RAW hazard (q2)",
		   "(q2_1cycle + 2*q2_2cycle + sim_num_insn)/sim_num_insn", NULL);
...
#include "machine.def"
	default:
      ...
      /* ECE552 Assignment 1 - BEGIN CODE*/
      // We want to add code that keeps track of clock cycles 
      // elapsed since the previous instruction
      int i, j;
      int q1_max_delay = 0;
      int q2_max_delay = 0;
      for (i = 0; i < 3; i++) {
          if (r_in[i] != DNA && reg_ready [r_in [i]] > sim_num_insn) {
              ...
              if ((reg_ready [r_in [i]] - sim_num_insn) == 2) {
	              q1_max_delay = 2;
                  q1_2cycle++; 
	              // stalling two cycles clears dependence on any other registers
        		  for (j = 0; j < MD_TOTAL_REGS; j++) {
		              reg_ready [j] = sim_num_insn;
		          }
                  break; // two cycle stall is maximum wait time
              }
              else if ((reg_ready [r_in [i]] - sim_num_insn) == 1) {
                  q1_max_delay = 1; 
              }
          }
      }
      ...
      // One Cycle Stall Case
      if (q1_max_delay == 1) {
	    for (j = 0; j < MD_TOTAL_REGS; j++) {
	      // if we stall one cycle, then registers that previously needed 
          // two stalls, now only need one
	      reg_ready [j] = (reg_ready[j]>(sim_num_insn+1))? sim_num_insn + 1 : sim_num_insn;
        }
        q1_1cycle++;
      }  
      
    //q2 - Reuses some of the code from Q1
    for (i = 0; i < 3; i++) {
	//store without using alu computation   
        if (r_in[i] != DNA && q2_reg_ready [r_in [i]] > sim_num_insn) {
        	...
        	//Dependency was a LOAD
    	    if ((q2_reg_ready[r_in [i]] - sim_num_insn) == 2) {
        	...
        	if ((i == 0) && (MD_OP_FLAGS(op) & F_MEM) &&
                ...
                // stalling two cycles clears dependence on any other registers
                for (j = 0; j < MD_TOTAL_REGS; j++) {
                    q2_reg_ready [j] = sim_num_insn;
                }
                break;
            }
            else if ((q2_reg_ready [r_in [i]] - sim_num_insn) == 1)
            {
    ...
    if (q2_max_delay == 1) {
	    for (j = 0; j < MD_TOTAL_REGS; j++) {
	        // if we stall one cycle, then registers that previously needed 
            // two stalls, now only need one
            q2_reg_ready [j] = (q2_reg_ready[j]>(sim_num_insn+1))? 
                                sim_num_insn + 1 : sim_num_insn;
        }
        q2_1cycle++;  
    }
 
    // Q1: Must wait for all valid instructions to WB
    if (r_out[0] != DNA)
    ...
    if ((MD_OP_FLAGS(op) & F_MEM) && (MD_OP_FLAGS(op) & F_LOAD)) {
	    if (r_out[0] != DNA)
            q2_reg_ready[r_out[0]] = sim_num_insn + 3;
    	if (r_out[1] != DNA)
            q2_reg_ready[r_out[1]] = sim_num_insn + 3;
    }
    
    // Q2: Delay due to ALU
    if ((MD_OP_FLAGS(op) & F_ICOMP) || (MD_OP_FLAGS(op) & F_FCOMP)) {
        if (r_out[0] != DNA)
            q2_reg_ready[r_out[0]] = sim_num_insn + 2;
        if (r_out[1] != DNA)
            q2_reg_ready[r_out[1]] = sim_num_insn + 2;
    }
