	     ...
	     int prefetch_type)		/* prefetcher type */
{
  ...
  for (i = 0; i < RPT_TABLE_SIZE; i++) {
      cp->rpt_table[i].state = INIT;
      cp->rpt_table[i].stride = 0;
      cp->rpt_table[i].prev_addr = 0;
...
void stride_prefetcher(struct cache_t *cp, md_addr_t addr) {
	...
	int rpt_table_index = pc & (RPT_TABLE_SIZE-1);
	rpt_row * rpt_table_entry = &(cp->rpt_table[rpt_table_index]);
	if (rpt_table_entry->state != NO_PRED) {
		cache_access(cp, Read, (addr + rpt_table_entry->stride) & ~(cp->bsize - 1), NULL, cp->bsize, 0, NULL, NULL, 1);
	}
	// if (rpt_table_entry->state == INIT) {
	// 	printf("%d INIT %d\n", pc, rpt_table_index);
	// } else if (rpt_table_entry->state == STEADY) {
